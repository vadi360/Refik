...
| **AI Yedek** | Claude (opsiyonel) | Admin yapılandırması ile her görev için seçilebilir |
...
### 6.1 AI Stratejisi — Çift Motor (Minimax + Claude)
...
**Admin AI Yapılandırması:**
- Minimax ve Claude bağlantıları sistemde hazır olacak
- Admin panelden her görev için hangi AI'ın kullanılacağı seçilebilir
- **Default:** Tüm görevler Minimax üzerinden çalışır
- **İleride:** İstenirse Claude aktif edilir, her görev için ayrı ayrı yapılandırılabilir
...
### 6.2 AI Yönlendirme Kural Tablosu

| Görev | Varsayılan Model | Açıklama | Onay Gerekiyor mu? |
|-------|------------------|----------|-------------------|
| Tebligat metni okuma & özetleme | Minimax | Hızlı analiz | Hayır (otomatik) |
| Süre çıkarımı | Minimax | "15 gün içinde itiraz" çıkarımı | Evet |
| UYAP dosya özeti | Minimax | Belgelerin RAG ile özetlenmesi | Hayır |
| Aleyhe/lehe karar tespiti | Minimax | Karar yönü belirlenmesi | Evet |
| Dilekçe taslak | Minimax | Otomatik üretim | Evet |
| Dilekçe revizyon | Minimax | Revize döngüsü | Evet |
| İhtarname | Minimax | Otomatik üretim | Evet |
| İçtihat araştırma | Minimax | RAG destekli arama | Evet |
| Genel Q&A | Minimax | Basit sorular | Hayır |

**Admin panelinde:** Her görev için Minimax veya Claude seçimi yapılabilir. Varsayılan Minimax'tir.
...
| Değişiklik | Açıklama |
|------------|----------|
| Yazım hatası düzeltildi | "Miminal" → "Minimax" |
| Rusça ifade temizlendi | "Техник реализация" → "Teknik Uygulama" |
| Embedding model doğrulaması | "Minimax embed model" → "doğrulanacak" eklendi |
| Çift AI motoru | Minimax + Claude bağlantıları hazır, Admin'den yapılandırma |
| Default Minimax | Tüm görevler varsayılan olarak Minimax |
| Admin AI seçimi | Her görev için Minimax veya Claude Admin panelden seçilebilir |
